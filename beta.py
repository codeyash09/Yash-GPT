...
def AttentionHead(segments, targets, Wk, Wq, Wv, Wh1, Wh2, Wo_final): #segment length = dim
    global vectors 
    cont = True
    output = ""
    ...
    if(cont):
        ...
        for i in reversed(range(num_layers)):
            ...
            dQ = dQ.reshape(bSize, lengthOfSegment, dim) 
            dK = dK.reshape(bSize, lengthOfSegment, dim)
            dV = dV.reshape(bSize, lengthOfSegment, dim)
        ...
        global GLoss
        GLoss += Loss
        return Wk, Wq, Wv, Wh1, Wh2, Wo_final
...
if(train):
    for i in range(steps):
        ...
        idx_offsets = n.arange(dim)
        ind = indxs[start[:, n.newaxis] + idx_offsets]
        targets = indxs[start[:, n.newaxis] + idx_offsets + 1]

        Wk, Wq, Wv, Wh1, Wh2, Wo_final = AttentionHead(ind, targets, Wk, Wq, Wv, Wh1, Wh2, Wo_final)
